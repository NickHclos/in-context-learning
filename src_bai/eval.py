    ...
    for _, r in df.iterrows():
        ...
        for eval_name, results in sorted(metrics.items()):
            ...
            for model_name, m in results.items():
                if "Encoder" in model_name in model_name:
                    model_name = r.model
...
if __name__ == "__main__":
    ...
    run_dir = sys.argv[1]
    #run_dir = "in-context-learning/encoder_final"
    print(run_dir)
    df = read_run_dir(run_dir)
    print(df)
    task = "linear_and_logistic_d=20"
    run_id = "749395b1-dab3-436f-b206-e9405832e251"
    run_path = os.path.join(run_dir, task, run_id)
    get_run_metrics(run_path, recompute=True)
    metrics = collect_results(run_dir, df)
    _, conf = get_model_from_run(run_path, only_conf=True)
    n_dims = conf.model.n_dims

#    print(metrics["linear_classification"].keys())
    
    for cur in metrics.keys() :
        models = relevant_model_names[cur]
        basic_plot(metrics[cur], models=models)
        plt.savefig(cur + ".png")
